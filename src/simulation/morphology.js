import { connectedComponents, foodPreference, effectiveFoodIntakeRate } from './sim.js?v=shared-behaviour-1';
...
export function findFeedingReshape(state, rules, world, allowPreparation = true, foodContext = null) {
  ...
  foodContext ??= {
    edible: edibleFoodKeys(state, rules),
    food: state.food.filter(entry => entry.amount >= (rules.foodSenseMinBiomass ?? 0.000001)),
    values: new Map(state.food.map(f => [key(f.x, f.y), foodPreference(f, rules, intakeRate)])),
    distances: new Map(),
  };
  const edible = foodContext.edible;
  const beforeContacts = state.cells.filter(c => edible.has(key(c.x, c.y))).length;
  const reachRadius = config.reachRadius ?? 3;
  const distanceToFood = (x, y) => {
    const position = key(x, y);
    if (!foodContext.distances.has(position)) foodContext.distances.set(position,
      foodContext.food.reduce((distance, entry) => Math.min(distance, Math.abs(entry.x - x) + Math.abs(entry.y - y)), reachRadius + 1));
    return foodContext.distances.get(position);
  };
  const neighbours = (x, y) => DIRS.filter(d => occupied.has(key(x + d.dx, y + d.dy))).length;
  const beforeExposed = state.cells.reduce((sum, c) => sum + 4 - neighbours(c.x, c.y), 0);
  ...
  for (const cell of cells) {
    ...
    const moves = [...DIRS, { dx: 1, dy: 1 }, { dx: 1, dy: -1 }, { dx: -1, dy: 1 }, { dx: -1, dy: -1 }];
    for (const { dx, dy } of moves) {
      ...
      if (recent?.cellId === cell.id && state.tick - recent.tick < 8
        && x === recent.from.x && y === recent.from.y) continue;

      const afterContacts = beforeContacts + Number(edible.has(destinationKey)) - Number(edible.has(key(cell.x, cell.y)));
      const contactGain = afterContacts - beforeContacts;
      if (contactGain < 0) continue;
      // A preparatory move only needs one valid follow-up that gains contact.
      if (!allowPreparation && contactGain === 0) continue;
      // Only geometry changes during a candidate evaluation; share read-only food/world data.
      const moved = { ...cell, x, y };
      const candidateState = { ...state, cells: state.cells.map(c => c.id === cell.id ? moved : c) };
      if (!candidateIsConnected(candidateState)) continue;
      const adjacentToOld = Math.abs(dx) + Math.abs(dy) === 1 ? 1 : 0;
      const afterExposed = beforeExposed + 2 * (neighbours(cell.x, cell.y) - (neighbours(x, y) - adjacentToOld));
      const exposedIncrease = afterExposed - beforeExposed;
      const approach = distanceToFood(cell.x, cell.y) - distanceToFood(x, y);
      ...
      if (!contactGain && approach <= 0 && !(beforeContacts > 0 && approach === 0 && exposedIncrease < 0)) {
        if (!allowPreparation || approach < 0 || beforeContacts === 0) continue;
        const followup = findFeedingReshape(candidateState, rules, world, false, foodContext);
      ...
      const feedingGain = (foodContext.values.get(destinationKey) ?? 0) - (foodContext.values.get(key(cell.x, cell.y)) ?? 0);
      const score = feedingGain * foodWeight + approach * (config.approachWeight ?? 3) - exposedIncrease * compactnessWeight + preparationGain;
      if (score <= 0) continue;
      ...
      if (!allowPreparation) return candidate;
